...
class _GoldenModel(torch.nn.Module):
    def forward(
        ...
        scores = torch.matmul(query, key.transpose(-1, -2)) * _SCALE
...
def case_definition() -> CaseDefinition:
    ...
    return CaseDefinition(
        ...
        inputs={
            ...
            "key": torch.randn(2, 2, 5, 8, generator=generator),
            "value": torch.randn(2, 2, 5, 8, generator=generator),
            "atten_mask": mask,
        ...
        description="BNSD 浮点 GQA、广播 mask、有效序列长度的确定性 Attention 用例。",
    )
