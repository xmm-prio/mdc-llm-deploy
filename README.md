...
非 TTY 环境（如 CI 或重定向输出）仍保留原有英文纯文本日志和无颜色静态阶段表，但自动抑制
动态进度刷新，不输出 ANSI 控制符或回车刷新字符。

如环境中存在 `NO_COLOR`（值可为空或任意内容），或 `TERM` 的值为 `dumb`（不区分大小写），
TTY 输出也会禁用颜色。该设置只影响颜色，不关闭已启用的 TTY 动态进度。流不支持
`isatty()` 或探测时发生异常，也会按非 TTY 处理。
