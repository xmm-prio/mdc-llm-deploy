...
raw QDQ 导出支持上述完整矩阵。`OnnxAdapter` 当前只 lowering 其中的 W8A8 子集：
...
`OnnxAdapter` 默认显示各处理阶段的 Rich 进度。非交互环境可使用
`OnnxAdapter(AdapterConfig(show_progress=False))(model)`，阶段日志不受该开关影响。

## Qwen3-8B 单层精度验证

`examples.qwen3_8b_layer_accuracy` 固定验证 Qwen3-8B 第 1 个 DecoderLayer 的 seq=128
Prefill，比较以下配置：

- FP16；
- 权重对称 per-channel、激活对称静态 per-token 的 W8A8；
- 权重对称 per-channel、激活对称静态 per-tensor 的 W8A8。

校准和评估使用不同真实文本。生成结果同时记录各 `MinMaxLinear` 的 scale 范围和评估集
激活饱和率。精度需要分两类解释：

1. W8A8 Torch 对 FP16 Torch 描述量化本身引入的模型误差；
2. MDC 对同配置 Torch 描述 ONNX lowering、ATC 与硬件执行误差，验收门槛为
   `cosine >= 0.999`。

两类误差不能混成一个指标。尤其不能要求 W8A8 输出与 FP16 输出满足板端同配置门槛，也
不能用改善校准数据掩盖 lowering 或硬件误差。

per-token 模式的每个 Linear 都保存 `[1, 128, 1]` scale。该模型不能接收其他 token
长度；不同 Prefill 长度和 Decode 必须分别校准、导出。

## 非目标

当前只支持 Dense `nn.Linear`。不处理 Qwen3 MoE experts 的 3D 参数，不提供 MoE 端到端
量化，公共量化 API 不负责硬件验证或质量基准，也不提供独立 ONNX 文件写入 API。
